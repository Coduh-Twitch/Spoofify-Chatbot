...
if (!isChannelAdded(process.env.BOT_USER_ID)) {
  addChannel({
    broadcasterId: process.env.BOT_USER_ID,
    broadcasterName: process.env.BOT_USER_NAME,
  });
}
...
export let chat: ChatClient | null = new ChatClient({
  authProvider,
  channels: channelsToArray(),
  isAlwaysMod: false,
});
...
export async function say(
  ...
  let clientUser = await apiClient.users.getAuthenticatedUser(
  ...
  let d: any = {
    ...
    message: `${withRandomizer ? `[${Math.floor(Math.random() * 1000)}] ` : ""}${content}`,
  };
  if (replyId) d.reply_parent_message_id = replyId;

  try {
    let r: any = await post(`https://api.twitch.tv/helix/chat/messages`, d, {
      headers: {
...
export async function reply(channel, content, replyId): Promise<string | null> {
  return await say(channel, content, replyId);
}

export async function permissionMessage(
  channel,
  replyId: string | null = null,
): Promise<string | null> {
  if (replyId) {
...
async function beerioGet(
  endpoint: string,
  requesterId: string,
): Promise<any | null> {
  let botSession = getSessionByUserId(process.env.BOT_USER_ID);
  if (!botSession) return null;
  let res: any = await get(`${process.env.BEERIO_WEB_URL}/api${endpoint}`, {
    headers: {
      "x-requester-id": requesterId,
      Cookie: `token-0=${botSession.accessToken}`,
    },
  });
...
async function initChat(c: ChatClient): Promise<void> {
  ...
  c.onMessage(async (channel, user, text, msg) => {
    ...
    if (
      msg.rewardId &&
      channel !== process.env.BOT_USER_NAME &&
      !rewardsInQuestion
    ) {
      ...
      if (rewardIsSongRequest(msg.rewardId)) {
        ...
        let searchingMsgId = await reply(
          channel,
          `Searching for ${query}...`,
          msg.id,
        );
        ...
        search(query)
          .then(async (searchResults) => {
            ...
            let addingMsgId = await reply(
              channel,
              `Adding "${result.name} - ${formatArtists(result.artists)}" to the queue...`,
              searchingMsgId,
            );
            ...
            setTimeout(async () => {
              if (added) {
                await reply(
                  channel,
                  `Added "${result.name} - ${formatArtists(result.artists)}" to the queue!`,
                  addingMsgId,
                );
              } else {
                await reply(
                  channel,
                  `Failed to add "${result.name} - ${formatArtists(result.artists)}" to the queue`,
                  addingMsgId,
                );
              }
            }, 1e3);
    ...
    if (channel === process.env.BOT_USER_NAME) {
      if (text.toLowerCase().trim() === "!join") {
        if (isChannelAdded(msg.userInfo.userId)) {
          await reply(
            channel,
            `🎸 The bot is already added to your channel!`,
            msg.id,
          );
        } else {
          addChannel({
            broadcasterId: msg.userInfo.userId,
            broadcasterName: msg.userInfo.userName,
          });
          await c.join(msg.userInfo.userName);

          await reply(
            channel,
            `🎸 The bot has been added to your channel!`,
            msg.id,
          );
          await say(
            user,
            `🎸  ShortBotduh is here! 💎 The bot may get caught by spam filters. Add VIP or Mod to the bot to bypass this. ❗ @${user} or a moderator must run !setup to begin using song request features.`,
            null,
          );
          apiClient.whispers
            .sendWhisper(
              process.env.BOT_USER_ID,
              msg.userInfo.userId,
              `Thank you for using Spoofify! Please log in at ${process.env.SPOOFIFY_WEB_URL}/auth to start using the bot! You can run !leave in the same channel to deactivate the bot. Happy listening!`,
            )
            .then(async () => {
              await apiClient.moderation.deleteChatMessages(msg.channelId);
            })
            .catch((e) => {
              console.log(e);
            });
        ...
        if (!isChannelAdded(msg.userInfo.userId)) {
          await reply(
            channel,
            `🎸 The bot is not added to your channel.`,
            msg.id,
          );
        } else {
          ...
          await reply(
            channel,
            `🎸 The bot has been removed from your channel.`,
            msg.id,
          );
        }
      }
      ...
      if (text.startsWith(prefix)) {
        ...
        if (
          ["nowplaying", "song", "np", "currentsong", "whatsplaying"].includes(
            command,
          )
        )
          await nowPlayingCommand(
        ...
        if (["queue", "q", "songlist", "tracklist"].includes(command))
          await queueCommand(
            userHasAuthority(msg.userInfo),
            channel,
            user,
        ...
        if (
          ["songrequest", "sr", "request", "addsong", "play"].includes(command)
        )
          await songRequestCommand(
            userHasAuthority(msg.userInfo),
            channel,
            user,
            text,
        ...
        if (["togglesr", "togglecommand"].includes(command))
          await toggleSongRequestCommand(
            userHasAuthority(msg.userInfo),
            channel,
            user,
            text,
        ...
        if (["addreward"].includes(command))
          await addRewardCommand(
            userHasAuthority(msg.userInfo),
            channel,
            user,
            text,
        ...
        if (["delreward", "removereward", "remreward"].includes(command))
          await removeRewardCommand(
            userHasAuthority(msg.userInfo),
            channel,
            user,
            text,
        ...
        if (["spoofify"].includes(command))
          await spoofifyCommand(
            userHasAuthority(msg.userInfo),
            channel,
            user,
            text,
...
app.get("/auth", async (req, res) => {
  if (req.query.code) {
    ...
    res.redirect(
      `https://id.twitch.tv/oauth2/authorize?client_id=${process.env.CLIENT_ID}&redirect_uri=${process.env.WEB_APP_URL}/auth&force_verify=true&scope=${scopes.join(" ")}&response_type=code`,
    );
  }
});
        ...
        if (clientAuthDetail.data?.data?.[0]) {
          ...
          if (!authProvider.hasUser(ud.id)) {
            if (botSession.userId === ud.id)
              authProvider.addUser(ud.id, botSession, ["chat"]);
          } else {
            if (!chat.isConnected) {
              chat.connect();
              await initChat(chat);
